...
//	
//	__shared__ float p[4][2], xj[3][2], f[4];
//	float g0, g1, g2, p1, p2, p12, sp, sm, rp, rm, temp, det;
//    float cxg, cyg, czg, cxp, cyp, czp;
//    float j1, j2, j3;
//	float co[3][4];
                   ...
                   )
{
	...
	dim3 numBlocks(*npg, *npg);
	cudaError_t error;

	thrust::complex<float> zhelem2[3][3];
	thrust::complex<float> zgelem2[3][3];
	...
	error = cudaFree(device_zhelem);
	cuda_assert(error);
	error = cudaFree(device_zgelem);
	cuda_assert(error);
